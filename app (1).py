...
form.addEventListener('submit',async e=>{e.preventDefault();if(busy||!input.value.trim())return;const text=input.value.trim();input.value='';input.style.height='auto';busy=true;form.querySelector('.send').disabled=true;try{await postText(text)}catch(err){bubble('Ops, não consegui enviar agora. Tenta de novo?',false)}finally{busy=false;if(!input.disabled)form.querySelector('.send').disabled=false;input.focus();}});
