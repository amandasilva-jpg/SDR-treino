...
CHAT_TEMPLATE = r'''
<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no,viewport-fit=cover"><title>Treino SDR</title>
<style>
:root{--blue:#0a4a9e;--blue-dark:#073d7f;--blue-light:#1a6fd0;--bright:#1273d4;--bg:#eef4fb;--ink:#162a47;--accent:#e8f1ff}*{box-sizing:border-box}body{margin:0;background:#c5d6ee;font-family:Arial,Helvetica,sans-serif;color:var(--ink);min-height:100vh;display:flex;justify-content:center}.phone{width:min(100%,540px);max-width:100%;height:100vh;margin:0 auto;background:var(--bg);display:flex;flex-direction:column;box-shadow:0 0 28px #0002;overflow:hidden}.top{height:66px;background:var(--blue);color:white;display:flex;align-items:center;padding:10px 16px;gap:12px;flex:none}.avatar{width:43px;height:43px;border-radius:50%;background:#d4e8ff;color:#073d7f;display:grid;place-items:center;font-size:22px}.top h1{font-size:16px;margin:0 0 4px}.status{font-size:12px;opacity:.85}.restart{margin-left:auto;border:0;background:transparent;color:#d4e8ff;font-size:11px;text-decoration:underline;cursor:pointer;padding:6px}.progress{height:4px;background:#073d7f;flex:none}.progress i{display:block;height:100%;background:var(--blue-light);width:0;transition:width .3s}.messages{padding:15px 12px 18px;overflow-y:auto;flex:1;min-height:0;background-color:var(--bg);background-image:radial-gradient(#d8d0c5 1px,transparent 1px);background-size:18px 18px}.row{display:flex;margin:5px 0}.row.me{justify-content:flex-end}.bubble{max-width:84%;padding:9px 11px 7px;border-radius:8px;background:#fff;box-shadow:0 1px 1px #0001;font-size:14px;line-height:1.42;white-space:pre-wrap;word-break:break-word}.me .bubble{background:#d4e8ff;border-top-right-radius:2px}.mentor .bubble{border-top-left-radius:2px}.time{display:block;text-align:right;color:#81908b;font-size:10px;margin-top:4px}.composer{background:#e8f1ff;padding:9px 10px max(9px, env(safe-area-inset-bottom));display:flex;gap:8px;align-items:flex-end;flex:none}.composer-default{display:flex;gap:8px;align-items:flex-end;width:100%}.composer textarea{border:0;resize:none;border-radius:22px;padding:12px 15px;font:16px Arial;line-height:1.25;min-height:44px;max-height:100px;flex:1;outline:none}.mic,.send{border:0;width:44px;height:44px;min-width:44px;border-radius:50%;color:white;font-size:20px;cursor:pointer;display:grid;place-items:center}.mic{background:var(--blue);font-size:19px}.send{background:var(--blue-light)}.mic:disabled,.send:disabled{background:#7ab5f0;cursor:default}.recording-overlay{display:none;position:fixed;bottom:0;left:50%;transform:translateX(-50%);width:min(100%,540px);background:#073d7f;padding:16px 20px;flex-direction:column;align-items:center;gap:10px;z-index:999;box-shadow:0 -4px 20px rgba(0,0,0,.3)}.recording-overlay.show{display:flex}.rec-overlay-bar{display:flex;align-items:center;gap:10px;width:100%;justify-content:center}.rec-overlay-dot{width:12px;height:12px;border-radius:50%;background:#e53935;animation:recpulse 1s infinite}.rec-overlay-timer{font-size:20px;font-weight:bold;color:#fff;font-variant-numeric:tabular-nums}.rec-overlay-hint{font-size:13px;color:#aaa;text-align:center}.rec-overlay-preview{font-size:12px;color:#888;max-width:100%;text-align:center;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}@keyframes recpulse{0%,100%{opacity:.3;transform:scale(.8)}50%{opacity:1;transform:scale(1.15)}}.mic.recording-active{background:#e53935;animation:recpulse 1s infinite}.audio-support{font-size:11px;text-align:center;color:#687871;padding:0 8px 7px;width:100%}.voice-bubble{min-width:220px;padding:9px 10px 7px}.voice-controls{display:flex;align-items:center;gap:9px}.voice-play{border:0;background:var(--blue);color:#fff;border-radius:50%;width:34px;height:34px;min-width:34px;font-size:15px;cursor:pointer}.voice-progress{height:5px;background:#a8c8ee;border-radius:5px;overflow:hidden;flex:1;cursor:pointer}.voice-progress i{display:block;height:100%;width:0;background:var(--blue);transition:width .1s}.voice-duration{font-size:11px;color:var(--blue);min-width:30px;text-align:right;font-variant-numeric:tabular-nums}.voice-native{width:100%;height:30px;margin-top:6px}.send:disabled{background:#7ab5f0;cursor:default}.typing{display:none;padding:8px 14px;flex:none}.typing.show{display:flex;gap:4px}.typing b{width:6px;height:6px;border-radius:50%;background:#87928d;animation:blink 1s infinite}.typing b:nth-child(2){animation-delay:.15s}.typing b:nth-child(3){animation-delay:.3s}@keyframes blink{0%,60%,100%{opacity:.25}30%{opacity:1}}.notice{font-size:12px;text-align:center;color:#6b7770;padding:7px}.done{padding:14px;text-align:center;background:#e8f1ff;font-size:13px;color:#53635d;flex:none}.done strong{display:block;color:var(--blue);margin-bottom:4px}@media(max-width:480px){.rec-overlay-hint{font-size:12px}}}
</style></head><body><main class="phone"><header class="top"><div class="avatar">👊</div><div><h1>Treino SDR</h1><div class="status" id="status">mentor de prospecção</div></div><button class="restart" id="restart" type="button">Recomeçar</button></header><div class="progress"><i id="progress"></i></div><section class="messages" id="messages"></section><div class="typing" id="typing"><b></b><b></b><b></b></div><div id="done"></div><form class="composer" id="composer"><div class="composer-default" id="composerDefault"><textarea id="input" rows="1" placeholder="Digite uma mensagem..." autocomplete="off"></textarea><button class="send" id="send" type="submit" aria-label="Enviar">➤</button><button class="mic" id="mic" type="button" aria-label="Gravar áudio" title="Gravar áudio">🎤</button></div><div class="recording-overlay" id="recordingOverlay"><div class="rec-overlay-bar"><span class="rec-overlay-dot"></span><span class="rec-overlay-timer" id="recordingTimer">0:00</span></div><div class="rec-overlay-hint" id="recordingHint">Segure para gravar · solte para enviar</div><div class="rec-overlay-preview" id="transcriptPreview"></div></div><div class="audio-support" id="audioSupport" hidden></div></form></main>
<script>
const messagesEl=document.getElementById('messages'), input=document.getElementById('input'), form=document.getElementById('composer'), composerDefault=document.getElementById('composerDefault'), mic=document.getElementById('mic'), recordingOverlay=document.getElementById('recordingOverlay'), recordingTimer=document.getElementById('recordingTimer'), transcriptPreview=document.getElementById('transcriptPreview'), audioSupport=document.getElementById('audioSupport'), typing=document.getElementById('typing'), doneEl=document.getElementById('done'), progress=document.getElementById('progress'), statusEl=document.getElementById('status'), restartEl=document.getElementById('restart'), send=document.getElementById('send');send.style.visibility='visible';if(!input.value.trim()){send.style.display='none';mic.style.display='grid';}else{send.style.display='grid';mic.style.display='none';}
...
function resetRecordingUI(){clearInterval(recordingInterval);recordingInterval=null;recordingOverlay.classList.remove('show');mic.classList.remove('recording-active');mic.disabled=false;recordingTimer.textContent='0:00';transcriptPreview.textContent='';}
...
let isRecording=false,touchStartX=0,touchStartY=0;
function startHoldRecording(e){e.preventDefault();if(!SpeechRecognition){showAudioNotice('Áudio disponível no Chrome. Tente pelo Chrome, ou digite sua resposta!');return;}if(!navigator.mediaDevices||!navigator.mediaDevices.getUserMedia||!window.MediaRecorder){showAudioNotice('Seu navegador não suporta áudio. Tente pelo Chrome, ou digite sua resposta!');return;}if(e.touches&&e.touches[0]){touchStartX=e.touches[0].clientX;touchStartY=e.touches[0].clientY;}isRecording=true;beginRecording();}
function stopHoldRecording(e){if(!isRecording)return;isRecording=false;finishRecording('send');}
function cancelHoldRecording(){if(!isRecording)return;isRecording=false;finishRecording('cancel');resetRecordingUI();}
mic.addEventListener('pointerdown',startHoldRecording);
document.addEventListener('pointerup',stopHoldRecording);
document.addEventListener('pointercancel',cancelHoldRecording);
mic.addEventListener('pointerleave',()=>{if(isRecording)cancelHoldRecording();});
document.addEventListener('pointermove',e=>{if(!isRecording)return;const dy=Math.abs(e.clientY-touchStartY);const dx=Math.abs(e.clientX-touchStartX);if(dy>80||dx>80)cancelHoldRecording();});
load();
</script></body></html>
'''

ADMIN_TEMPLATE = r'''
<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Treino SDR — Admin</title><style>body{font-family:Arial,sans-serif;margin:0;padding:28px;background:#f5f7f6;color:#21332f}main{max-width:1400px;margin:auto}h1{color:var(--blue);margin:0 0 8px}.sub{color:#60716a;margin:0 0 22px}.cards{display:flex;gap:12px;margin-bottom:20px}.card{background:white;border:1px solid #dce5e0;border-radius:10px;padding:15px 20px;min-width:180px}.num{font-size:27px;font-weight:bold;color:var(--blue);display:block}.label{font-size:12px;color:#677770}a.button{display:inline-block;background:var(--blue);color:#fff;text-decoration:none;border-radius:7px;padding:10px 14px;font-size:13px;margin-bottom:14px} .table-wrap{background:#fff;border:1px solid #dce5e0;border-radius:10px;overflow:auto}table{border-collapse:collapse;width:100%;font-size:13px;min-width:1060px}th,td{text-align:left;padding:11px 12px;border-bottom:1px solid #edf1ef;vertical-align:top}th{background:#eef7f3;color:var(--blue);font-size:11px;text-transform:uppercase;letter-spacing:.03em;white-space:nowrap}tr:last-child td{border-bottom:0}.score{font-weight:bold;color:var(--blue)}.obs{max-width:280px;white-space:pre-line;line-height:1.35}.pill{padding:4px 7px;border-radius:12px;background:#fff3d6;color:#916b13;font-size:11px;white-space:nowrap}.pill.done{background:#d9f7e5;color:#073d7f}</style></head><body><main><h1>Treino SDR</h1><p class="sub">Painel de pré-qualificação · dados mantidos em memória nesta execução</p><div class="cards"><div class="card"><span class="num">{{ evaluated }}</span><span class="label">candidatos avaliados</span></div><div class="card"><span class="num">{{ ongoing }}</span><span class="label">em andamento</span></div></div><a class="button" href="/admin/export.json">Exportar JSON</a><div class="table-wrap"><table><thead><tr><th>Nome</th><th>Contato</th><th>Etapa</th><th>Status</th><th>Clareza</th><th>Raciocínio</th><th>Resiliência</th><th>Engajamento</th><th>Geral</th><th>Observações</th><th>Início</th><th>Conclusão</th></tr></thead><tbody>{% for c in candidates %}<tr><td>{{ c.name or '—' }}</td><td>{{ c.contact or '—' }}</td><td>{{ c.stage_label }}</td><td><span class="pill {{ 'done' if c.complete else '' }}">{{ 'Concluído' if c.complete else 'Em andamento' }}</span></td><td class="score">{{ c.scores.clareza }}</td><td class="score">{{ c.scores.raciocinio }}</td><td class="score">{{ c.scores.resiliencia }}</td><td class="score">{{ c.scores.engajamento }}</td><td class="score">{{ c.overall }}</td><td class="obs">{{ c.observations or 'Aguardando mais respostas.' }}</td><td>{{ c.started_at }}</td><td>{{ c.completed_at or '—' }}</td></tr>{% else %}<tr><td colspan="12">Nenhum candidato nesta execução.</td></tr>{% endfor %}</tbody></table></div></main></body></html>
'''
