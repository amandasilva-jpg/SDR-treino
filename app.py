...
function startHoldRecording(e){e.preventDefault();if(!SpeechRecognition){showAudioNotice('Áudio disponível no Chrome. Tente pelo Chrome, ou digite sua resposta!');return;}if(!navigator.mediaDevices||!navigator.mediaDevices.getUserMedia||!window.MediaRecorder){showAudioNotice('Seu navegador não suporta áudio. Tente pelo Chrome, ou digite sua resposta!');return;}if(e.touches){touchStartX=e.touches[0].clientX;touchStartY=e.touches[0].clientY;}isRecording=true;beginRecording();}
...
mic.addEventListener('mousedown',startHoldRecording);
mic.addEventListener('touchstart',startHoldRecording,{passive:false});
mic.addEventListener('mouseup',stopHoldRecording);
mic.addEventListener('touchend',stopHoldRecording);
mic.addEventListener('mouseleave',()=>{if(isRecording)cancelHoldRecording();});
mic.addEventListener('touchmove',e=>{if(!isRecording||!e.touches)return;const dy=Math.abs(e.touches[0].clientY-touchStartY);const dx=Math.abs(e.touches[0].clientX-touchStartX);if(dy>60||dx>60)cancelHoldRecording();},{passive:true});
window.addEventListener('mouseup',()=>{if(isRecording)stopHoldRecording();});
window.addEventListener('touchend',()=>{if(isRecording)stopHoldRecording();});
load();
</script></body></html>
'''
